...
class Rule extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
     ...
     * @param \Magento\Eav\Model\Entity\AbstractEntity $eavmodel
     * @param string $connectionName
     */
    public function __construct(
        \Magento\Framework\Model\ResourceModel\Db\Context $context,
        \Magento\Store\Model\StoreManagerInterface $storeManager,
        \Magento\Eav\Model\Entity\AbstractEntity $eavmodel,
        $connectionName = null
        ) {
     ...
     */
        ...
        {
            $connection = $this->getConnection();
            $select = $connection->select()->from($this->getTable('lof_couponcode_rule'))->join(array('salesrule' => $this->getTable('salesrule')), 'lof_couponcode_rule.rule_id = salesrule.rule_id')->where(
...
// ------------------------------------
        public function lookupRuleByid($id)
        {
            $connection = $this->getConnection();
            $select = $connection->select()->from($this->getTable('lof_couponcode_rule'))->join(array('salesrule' => $this->getTable('salesrule')), 'lof_couponcode_rule.rule_id = salesrule.rule_id')->where(
                'lof_couponcode_rule.rule_id = :rule_id'
                )->limit(1);
            $binds = [':rule_id' => (int)$id];
            return $connection->fetchRow($select, $binds);
        }
